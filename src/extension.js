...
/**
 * The LLM route the extension bakes into the profile's composition base.
 *
 * A brand-new DSH_HOME ships `agent-default-model` pointed at `deepseek-official`,
 * which is of no use without an official key; the user's working route lives in
 * settings.yaml, which this extension never writes. The composition base and the
 * user's settings layer merge per provider in `dsh-llm-pi-ai`, so baking the route
 * as composition config gives a fresh home a working model while their own
 * settings.yaml — where they have one — still wins field by field for that route.
 * @returns {object | null} null when the setting is unset, disabled, or incomplete.
 */
function bakedOpenCodeProvider() {
  const raw = vscode.workspace.getConfiguration("dsh").get("opencodeProvider");
  ...
  return {
    ...
    defaultProvider: typeof raw.defaultProvider === "string" && raw.defaultProvider.trim() !== "" ? raw.defaultProvider.trim() : route,
...
/**
 * Emit the overlay rows that carry the baked provider into the composition base:
 * the `llm-pi-ai` route set, and `agent-default-model` re-pointed at it.
 * @param {object} provider - validated by bakedOpenCodeProvider().
 * @returns {string[]} YAML lines, empty when there is nothing to say.
 */
function openCodeOverlayRows(provider) {
  if (!provider) return [];
  const scalar = (value) => yamlScalar(String(value));
  const lines = ["- id: llm-pi-ai", "  config:", "    providers:", `      ${provider.route}:`];
  if (provider.apiKeyEnv !== "") lines.push(`        apiKeyEnv: ${scalar(provider.apiKeyEnv)}`);
  if (provider.api !== "") lines.push(`        api: ${scalar(provider.api)}`);
  lines.push(`        baseURL: ${scalar(provider.baseURL)}`);
  lines.push("        models:");
  for (const model of provider.models) {
    if (typeof model === "string") { lines.push(`          - id: ${scalar(model)}`); continue; }
    if (model === null || typeof model !== "object" || typeof model.id !== "string" || model.id.trim() === "") continue;
    lines.push(`          - id: ${scalar(model.id)}`);
    for (const [key, value] of Object.entries(model)) {
      if (key === "id") continue;
      if (Array.isArray(value)) lines.push(`            ${key}: [${value.map((one) => scalar(one)).join(", ")}]`);
      else if (value !== null && typeof value === "object") {
        lines.push(`            ${key}: {${Object.entries(value).map(([k, v]) => `${scalar(k)}: ${scalar(v)}`).join(", ")}}`);
      } else if (typeof value === "number" || typeof value === "boolean") lines.push(`            ${key}: ${value}`);
      else if (typeof value === "string" && value !== "") lines.push(`            ${key}: ${scalar(value)}`);
    }
  }
  if (provider.defaultModel !== "") {
    lines.push("- id: agent-default-model", "  config:", `    provider: ${scalar(provider.defaultProvider)}`, `    model: ${scalar(provider.defaultModel)}`);
  }
  return lines;
}
...
function writeProfileOverlay(home) {
  ...
  const provider = bakedOpenCodeProvider();
  const rows = [
    ...(families.length > 0
      ? ["- id: modlens", "  config:", "    families:", ...families.map((family) => `      - ${yamlScalar(family)}`)]
      : []),
    ...openCodeOverlayRows(provider)
  ];
  if (rows.length === 0) {
    try {
      if (fs.existsSync(overlayPath)) {
        fs.rmSync(overlayPath, { force: true });
        log("profile overlay removed (nothing is configured to bake)");
      }
    } catch { /* best effort */ }
    return null;
  }
  const body = [
    "# Generated by the dsh-vscode extension from its own settings (dsh.modlensFamilies",
    "# and dsh.opencodeProvider). Applied with `dsh --patch`, AFTER the profile layer, so",
    "# the profile's own cordis.patch.yml is never rewritten and user settings always win",
    "# over these composition-base values. Local edits here are overwritten on boot.",
    ...rows
  ].join("\n") + "\n";
  try {
    let current = null;
    ...
    if (current !== body) {
      ...
      log("profile overlay written: " + overlayPath + " (modlens families: " + families.join(", ")
        + (provider ? ` | baked provider: ${provider.route} -> ${provider.baseURL}` : "") + ")");
    }
    return overlayPath;
  } catch (error) {
...
async function activate(context) {
  ...
  const settingsWatcher = vscode.workspace.onDidChangeConfiguration?.((event) => {
    if (!event.affectsConfiguration("dsh.modlensFamilies") && !event.affectsConfiguration("dsh.opencodeProvider")) return;
    log("a setting that shapes the profile overlay changed -> restarting the host so it applies");
    void restartHost();
  });
