...
let DefaultSessionArchiveService = (() => {
    ...
    return class DefaultSessionArchiveService extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _tombstone_decorators, { kind: "method", name: "tombstone", static: false, private: false, access: { has: obj => "tombstone" in obj, get: obj => obj.tombstone }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _untombstone_decorators, { kind: "method", name: "untombstone", static: false, private: false, access: { has: obj => "untombstone" in obj, get: obj => obj.untombstone }, metadata: _metadata }, null, _instanceExtraInitializers);
            ...
            __esDecorate(this, null, _deleteAll_decorators, { kind: "method", name: "deleteAll", static: false, private: false, access: { has: obj => "deleteAll" in obj, get: obj => obj.deleteAll }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        archiveTimes = (__runInitializers(this, _instanceExtraInitializers), new Map());
        archiveTimesPath;
        tombstones = new Set();
        tombstonesPath;
        ...
        async [Service.init]() {
            ...
            await this.loadDocument(this.archiveTimesPath, (parsed) => {
                if (!isArchiveTimesFile(parsed))
                    throw new Error('rollback archive: archive timestamp file is invalid');
                for (const [id, archivedAt] of Object.entries(parsed.archivedAt))
                    this.archiveTimes.set(id, archivedAt);
            });
            await this.loadDocument(this.tombstonesPath, (parsed) => {
                if (!isTombstonesFile(parsed))
                    throw new Error('rollback archive: tombstone file is invalid');
        ...
        async untombstone(sessionId) {
            if (this.tombstones.delete(sessionId))
                await this.writeTombstones();
            return this.success({ changed: true });
        }
        ...
        async deleteAll() {
            ...
            for (const sessionId of this.ctx.workspaceRegistry.archivedSessionIds) {
                ...
                if (result.ok)
                    ...
                    failed.push({ sessionId, message: result.error.message });
            }
        ...
        async setArchiveTime(sessionId, archivedAt) {
            ...
            if (archivedAt === undefined)
                this.archiveTimes.delete(sessionId);
            else
                this.archiveTimes.set(sessionId, archivedAt);
            try {
                ...
                if (prior === undefined)
                    ...
                    this.archiveTimes.set(sessionId, prior);
                throw error;
            }
        }
...
export { DefaultSessionArchiveService };
...
function isArchiveTimesFile(value) {
    ...
    if (candidate.version !== 1 || candidate.archivedAt === null || typeof candidate.archivedAt !== 'object')
        return false;
...
function isTombstonesFile(value) {
    if (value === null || typeof value !== 'object')
        return false;
    const candidate = value;
    if (candidate.version !== 1 || !Array.isArray(candidate.sessionIds))
        return false;
