...
export declare const DOCTOR_PROTOCOL_VERSION = 1;
/** System-wide rescue/recovery phase. */
export type DoctorSystemPhase = 'disabled' | 'provisioning' | 'armed' | 'degraded' | 'updating' | 'rolling-back' | 'uninstalling' | 'broken';
/** Per-profile runtime phase. */
export type DoctorProfilePhase = 'idle' | 'starting' | 'healthy' | 'degraded' | 'stopping' | 'exited' | 'suspected' | 'failed' | 'quarantined';
/** Incident lifecycle phase. */
export type DoctorIncidentPhase = 'opened' | 'collecting' | 'rescue-starting' | 'rescue-active' | 'diagnosing' | 'plan-ready' | 'repairing' | 'candidate-testing' | 'awaiting-confirmation' | 'promoting' | 'recovered' | 'rolled-back' | 'unresolved';
/** Incident kind reported by the supervisor. */
export type DoctorIncidentKind = 'boot-failure' | 'process-crash' | 'heartbeat-timeout' | 'http-failure' | 'client-failure' | 'dependency-failure' | 'configuration-failure';
...
export type DoctorActionName = 'provision' | 'exercise' | 'diagnose' | 'repair' | 'confirm' | 'rollback' | 'pause' | 'resume' | 'uninstall';
/**
