...
export interface TaskRecord {
  ...
  /**
   * Model the execution session must select (`<provider>:<model>`); absent
   * keeps the deployment/session default.
   */
  model?: string
  /**
...
export interface NewTaskInput {
  ...
  permission?: TaskPermission
  /** Model the execution session must select (`<provider>:<model>`); empty/absent = session default. */
  model?: string
  /**
