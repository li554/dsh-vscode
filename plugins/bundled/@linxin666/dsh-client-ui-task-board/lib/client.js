	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			"exec.model.default": "会话默认",
			"detail.executionSettings": "执行设置",
			"exec.hint": "执行时生效：工作区决定执行会话落在哪个工作区；模式决定会话的 agent 预设；权限经 /permission 命令应用到会话；模型决定执行会话选用的模型。留空则使用运行时默认。",
			"settings.title": "任务看板",
		...
		const en = {
			...
			"exec.model.default": "Session default",
			"detail.executionSettings": "Execution Settings",
			"exec.hint": "Applied when the task runs: the workspace decides where the execution session lands; the mode composes the session's agent preset; the permission is applied through the /permission command; the model selects the execution session's model. Blank = runtime default.",
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								children: options.models.length > 0 ? [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
									value: "",
									children: t("exec.model.default")
								}), ...options.models.flatMap((group) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("optgroup", {
									label: group.name,
									children: group.models.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("option", {
										value: `${group.id}:${option.id}`,
										children: option.name
									}, option.id))
								}, group.id))] : [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
									value: "",
									children: t("exec.model.default")
								})]
							})]
		...
		*/
		...
		function ExecutionSettingsSection({ controller, task, pending }) {
			...
			const modelKnown = model === "" || options.models.some((group) => group.models.some((item) => `${group.id}:${item.id}` === model));
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							...
							children: [
								...
								!modelKnown && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("option", {
									value: model,
									children: [model, t("exec.mode.removed")]
								}),
								options.models.length > 0 ? options.models.flatMap((group) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("optgroup", {
									label: group.name,
									children: group.models.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("option", {
										value: `${group.id}:${option.id}`,
										children: option.name
			...
			const mountUi = () => {
				...
				const pushModelOptions = async () => {
					try {
						...
						if (!response.result.ok) return;
						controller.setExecutionOptions({ models: response.result.value.groups.map((group) => ({
							id: group.id,
							name: group.name,
