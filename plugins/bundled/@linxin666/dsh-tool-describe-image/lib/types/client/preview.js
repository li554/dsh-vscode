 ...
 */
export function findImageReferences(text) {
    const matches = [];
 ...
 */
export function installConversationImagePreview(isEnabled, root) {
    ...
    const rememberFailure = (path) => {
        if (failedPaths.size >= MAX_FAILED_PATHS) {
            ...
            if (oldest.done !== true)
                failedPaths.delete(oldest.value);
        }
    ...
    const restoreAll = () => {
        ...
        if (within === undefined)
            return;
        for (const preview of within.querySelectorAll(`[${PREVIEW_ATTR}]`))
    ...
    const scanNode = (node) => {
        if (node.nodeType === Node.TEXT_NODE) {
            const text = node;
            if (text.data.includes('/describe-image/raw/') && !isExcluded(text))
                enhanceNode(text);
            return;
        }
        ...
        const walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT, {
            acceptNode: candidate => {
                ...
                if (!text.data.includes('/describe-image/raw/'))
