...
export const THINKING_SUFFIXES = ['off', 'low', 'medium', 'high'];
/** Per-call vision request timeout in milliseconds. */
export const DEFAULT_TIMEOUT_MS = 120_000;
/** Protocol styles the tool can speak to the configured endpoint. */
export const API_STYLES = ['chat-completions', 'responses', 'anthropic-messages'];
 ...
 */
export function splitModelSuffix(model) {
    const trimmed = model.trim();
    const match = /:(off|low|medium|high)$/.exec(trimmed);
    if (match === null)
        return { model: trimmed, thinking: undefined };
    return { model: trimmed.slice(0, -match[0].length), thinking: match[1] };
}
