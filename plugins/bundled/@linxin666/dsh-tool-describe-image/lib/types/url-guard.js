...
export function ipv6ToInt(ip) {
    ...
    if (hasDouble) {
        ...
        if (beforeValues.includes(undefined) || afterValues.includes(undefined))
            return undefined;
        const expansion = 8 - beforeValues.length - afterValues.length - tailGroups;
        if (expansion < 0)
            return undefined;
        for (const group of beforeValues)
            value = (value << 16n) | group;
        for (let index = 0; index < expansion; index += 1)
            value <<= 16n;
        for (const group of afterValues)
            value = (value << 16n) | group;
    }
    else {
        if (pieces.length + tailGroups !== 8)
            return undefined;
        for (const piece of pieces) {
    ...
    if (v4Tail !== undefined)
        value = (value << 32n) | v4Tail;
    return value;
}
/** Whether one IPv4 address value falls in a blocked CIDR. */
export function isBlockedIpv4Value(value) {
...
export function isBlockedIpv6Value(value) {
...
export function isLocalhostVariant(name) {
 ...
 * cannot be resolved.
 */
export async function assertImageUrlAllowed(rawUrl, resolve = systemResolver) {
    let url;
    try {
        url = new URL(rawUrl);
    }
    ...
    if (name === '' || isLocalhostVariant(name))
        throw new Error(IMAGE_URL_NOT_ALLOWED);
    ...
    if (name.includes(':')) {
        ...
        if (v6 === undefined || isBlockedIpv6Value(v6))
            throw new Error(IMAGE_URL_NOT_ALLOWED);
        return;
    }
    let addresses;
