...
import { registerAttachRoute, registerModelRoutes } from "./attach-routes.js";
import { DEFAULT_MAX_BYTES } from "./media.js";
import { createCapabilityProbe, createRouteResolver } from "./model-capability.js";
import { installToolVisibility } from "./tool-visibility.js";
import { registerNativeImageRoutes } from "./native-images.js";
import { Config, DESCRIBE_IMAGE_SETTINGS_NAMESPACE, resolveApiKey, resolveConfig } from "./config-resolve.js";
import { callVision, createVisionCache, loadImage } from "./vision-client.js";
...
export { DEFAULT_MAX_BYTES, sniffMimeType } from "./media.js";
export { API_STYLES, Config, DEFAULT_API_KEY_ENV, DEFAULT_API_STYLE, DEFAULT_MAX_OUTPUT_TOKENS, DEFAULT_INTERCEPT_IMAGE_SEND, DEFAULT_PROMPT, DEFAULT_RENDER_IMAGE_PREVIEW, DEFAULT_TIMEOUT_MS, DESCRIBE_IMAGE_SETTINGS_NAMESPACE, THINKING_SUFFIXES, resolveApiKey, resolveConfig, splitModelSuffix, } from "./config-resolve.js";
export { callVision, createVisionCache, DEFAULT_CACHE_MAX_ENTRIES, DEFAULT_CACHE_TTL_MS, extractChatCompletionsContent, extractResponsesContent, loadImage, parseImageAttachmentRef, readAttachment, readBoundedBody, readBoundedText, semanticRequestKey, } from "./vision-client.js";
export { buildModelsUrl, buildModelPingRequest, extractModelIds, handleModelProbe, handleModelTest, probeModels, testModelConnection, PROBE_MAX_BODY_BYTES, PROBE_MAX_MODELS, PROBE_MODEL_PLACEHOLDER, PROBE_TIMEOUT_MS, } from "./model-probe.js";
const DESCRIPTION_HEAD = 'Inspect one image — a local absolute path inside the session workspace, an http(s) URL, a complete `[image attachment ...]` note, '
...
function applyImpl(ctx, config = {}) {
    ...
    installSettingsSection(ctx, DESCRIBE_IMAGE_SETTINGS_NAMESPACE, Config, config, {
        ...
        validate: (value) => {
            ...
            if (value.baseURL !== undefined && value.model !== undefined)
                resolveConfig(value);
        },
    });
    ...
    if (webserver !== undefined) {
        for (const route of registerNativeImageRoutes(ctx, routeResolver))
