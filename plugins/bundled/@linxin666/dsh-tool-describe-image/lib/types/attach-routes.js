 ...
 */
import { attachmentMarkdown as renderAttachmentMarkdown, parseImageAttachmentRef } from "./attachment-reference.js";
import { decodeBase64, isImageMimeType, sniffMimeType, DEFAULT_MAX_BYTES } from "./media.js";
import { UNKNOWN_CAPABILITY } from "./model-capability.js";
import { handleModelProbe, handleModelTest } from "./model-probe.js";
 ...
 * exceeded it.
 */
export function attachBodyCap(maxBytes) {
    return Math.ceil(maxBytes / 3) * 4 + 1024;
}
/** The failure envelope used when a non-POST request hits the route. */
export const METHOD_NOT_ALLOWED = { code: 'internal', message: 'only POST is allowed' };
/**
 ...
 */
export function registerAttachRoute(ctx, readMaxBytes = () => DEFAULT_MAX_BYTES, probe) {
 ...
 */
export function registerModelRoutes(ctx, readConfig, resolveKey) {
    const webserver = ctx.get('webServer');
    if (webserver === undefined)
        return;
    webserver.register({
