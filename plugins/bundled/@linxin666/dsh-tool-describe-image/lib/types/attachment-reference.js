 ...
 */
import { isImageMimeType } from "./media.js";
/** Error text shown when a model-supplied attachment reference does not validate. */
export const ATTACHMENT_REF_GUIDANCE = 'describe-image: image is not a valid attachment reference; pass the complete [image attachment ...] note or generated Markdown reference';
...
function unwrapAttachmentNote(raw) {
    ...
    if (!trimmed.startsWith(ATTACHMENT_NOTE_PREFIX))
        return trimmed;
    if (!trimmed.endsWith(']'))
 ...
 */
export function parseMarkdownAttachmentReference(raw) {
...
export function attachmentMarkdown(refOrId) {
    ...
    if (typeof refOrId === 'string')
        return `![图片](/describe-image/raw/${encodedId})`;
