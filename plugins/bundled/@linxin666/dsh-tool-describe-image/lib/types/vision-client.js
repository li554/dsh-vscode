 ...
 */
export async function readAttachment(ctx, raw, signal) {
...
function toImage(bytes, source) {
    if (bytes.length === 0)
        throw new Error(`describe-image: image is empty: ${source}`);
 ...
 */
export async function loadImage(ctx, input, signal, maxBytes, workspace) {
    const trimmed = input.trim();
    if (trimmed.length === 0)
        throw new Error('describe-image: image must be a non-empty path, URL, or attachment reference');
    ...
    if (!info.isFile())
        throw new Error(`describe-image: image path is not a file: ${trimmed}`);
...
export async function readBoundedBody(response, cap) {
    const chunks = [];
    let total = 0;
    await drainResponse(response, (value) => {
        ...
        total += chunk.length;
        if (total > cap)
            throw new Error(`describe-image: response exceeds the ${cap}-byte bound`);
        chunks.push(chunk);
        return undefined;
    });
 ...
 */
export async function readBoundedText(response, cap) {
 ...
 */
export function extractChatCompletionsContent(payload) {
    const root = asRecord(payload);
    const choices = root?.choices;
    if (root === undefined || !Array.isArray(choices) || choices.length === 0)
        unexpectedShape();
...
export function extractResponsesContent(payload) {
    const root = asRecord(payload);
    const output = root?.output;
    if (root === undefined || !Array.isArray(output))
        unexpectedShape();
    const parts = [];
    for (const item of output) {
        ...
        if (type !== 'message' || role !== 'assistant' || !Array.isArray(content))
            continue;
...
function extractResponsesStreamContent(payloadBytes) {
    ...
    if (completedOutput !== undefined)
        return extractResponsesContent(completedOutput);
...
export function extractAnthropicMessagesContent(payload) {
    const root = asRecord(payload);
    const content = root?.content;
    if (root === undefined || !Array.isArray(content))
        unexpectedShape();
    const parts = [];
 ...
 */
export function buildVisionRequest(spec, prompt, image) {
...
export function semanticRequestKey(spec, prompt, image) {
...
export async function callVision(spec, apiKey, prompt, image, signal, cache) {
    if (cache !== undefined) {
    ...
    if (cache !== undefined)
        cache.set(semanticRequestKey(spec, prompt, image), text);
    return text;
}
