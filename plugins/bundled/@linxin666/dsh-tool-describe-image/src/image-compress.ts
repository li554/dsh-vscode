...
/** One decoded bitmap: dimensions plus tightly packed RGBA pixels. */
interface RgbaImage {
  width: number
  height: number
  data: Uint8Array
...
/** The parsed fields of one PNG's IHDR chunk. */
interface PngHeader {
  width: number
  height: number
  bitDepth: number
 ...
 */
export async function compressIfNeeded(image: { bytes: Buffer, mimeType: string }): Promise<{ bytes: Buffer, mimeType: string }> {
  ...
  try {
    if (bytes.length <= BYTE_TRIGGER) {
      ...
      if (size === undefined || Math.max(size.width, size.height) <= MAX_EDGE) return image
    }
    const decoded = mimeType === 'image/png' ? await decodePng(bytes) : await decodeJpeg(bytes)
    if (decoded === undefined) return image
...
function readPngHeader(bytes: Buffer): PngHeader | undefined {
  ...
  if (bytes[26] !== 0 || bytes[27] !== 0) return undefined
 ...
 */
function jpegSofSize(bytes: Buffer): { width: number, height: number } | undefined {
...
async function decodeJpeg(bytes: Buffer): Promise<RgbaImage | undefined> {
  try {
 ...
 */
async function decodePng(bytes: Buffer): Promise<RgbaImage | undefined> {
  const header = readPngHeader(bytes)
  if (header === undefined) return undefined
  const { width, height, bitDepth, colorType, interlace } = header
  if (interlace !== 0) return undefined
  const channels = PNG_CHANNELS[colorType]
  if (channels === undefined || (bitDepth !== 8 && bitDepth !== 16)) return undefined
  if (colorType === 3 && bitDepth !== 8) return undefined
  if (width * height > MAX_MEGAPIXELS * 1_000_000) return undefined
  ...
  while (offset + 12 <= bytes.length) {
    ...
    if (!/^[A-Za-z]{4}$/.test(type) || offset + 12 + length > bytes.length) return undefined
  ...
  if (colorType === 3 && (palette === undefined || palette.length === 0 || palette.length % 3 !== 0)) return undefined
  let raw: Uint8Array
  try {
  ...
  if (!unfilterPng(raw, height, rowBytes, bpp)) return undefined
...
function unfilterPng(raw: Uint8Array, height: number, rowBytes: number, bpp: number): boolean {
  let previous = -1
...
function rgbaFromPng(raw: Uint8Array, width: number, height: number, channels: number, bytesPerSample: number, colorType: number, palette: Buffer | undefined, trns: Buffer | undefined): RgbaImage | undefined {
  const bpp = channels * bytesPerSample
  const rowBytes = width * bpp
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (colorType === 6) {
        ...
        if (index >= paletteEntries) return undefined
...
function sampleAt(raw: Uint8Array, offset: number, bytesPerSample: number): number {
...
function flattenAlphaOntoWhite(rgba: Uint8Array): void {
...
function resizeBilinear(source: Uint8Array, width: number, height: number, targetWidth: number, targetHeight: number): Uint8Array {
