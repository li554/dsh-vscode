...
//#region src/index.ts
const name = "dsh-mnemon-strategy-auto-capture";
...
const memoryPlugin = defineMemoryPlugin({
	...
	description: {
		en: "Guide the current LLM to retain qualified durable facts.",
		"zh-CN": "引导当前 LLM 保留符合条件的长期事实。"
	},
	roles: ["strategy-extension"],
	provides: [{
		id: "strategy.default-three-tier.capture",
		exclusive: true
	}],
...
function createAutoCaptureExtension(config = {}) {
	const capture = validateThreeTierExtension("capture", {
		instruction: config.instruction ?? instruction,
		actionIds: config.actionIds ?? ["remember"],
		...config.sourceKeys === void 0 ? {} : { sourceKeys: config.sourceKeys }
	});
	return defineThreeTierExtension({
		typeId: "auto-capture",
		packageName: name,
		slot: "capture",
		contribute: (_request, sources) => ({
			...capture,
			sourceKeys: capture.sourceKeys ?? sources.filter((source) => source.role === "durable-evidence").map((source) => source.sourceInstanceKey).sort()
		})
	});
}
function apply(ctx, config = {}) {
	installMemory(ctx, {
		plugin: memoryPlugin,
		strategyExtensions: [createAutoCaptureExtension(config)]
	});
}
const memoryStrategyConfiguration = defineMemoryStrategyConfiguration({
	...
	description: {
		en: "Guide the current LLM to retain durable user-supplied facts; no background Agent.",
		"zh-CN": "引导当前 LLM 保留持久的用户事实，不启动后台 Agent。"
	},
	fields: [
		{
			key: "sourceKeys",
			input: "source-list",
			sourceRoles: ["durable-evidence"],
			label: {
				en: "Recording targets",
				"zh-CN": "记录目标"
			}
		},
		{
		...
		},
		{
			...
			label: {
				en: "Recording operation IDs",
				"zh-CN": "记录操作 ID"
			},
			description: {
				en: "Source-local operation IDs; this does not grant write permission.",
				"zh-CN": "填写 Source 内部的操作 ID；此配置不会授予写入权限。"
			}
		}
	],
	create: (config) => ({
...
//#endregion
export { apply, createAutoCaptureExtension, inject, memoryPlugin, memoryStrategyConfiguration, name };
