...
import { GIT_CHECKPOINT_FORMAT_VERSION, LEDGER_FORMAT_VERSION, } from './types.js';
...
export function parseManifest(value) {
    ...
    if (restorePointKind !== 'turn')
        corrupt('Git-native v2 restore points are automatic turn checkpoints');
...
function parseEntry(value, path, version) {
    ...
    if (kind === 'file') {
        ...
        if (version === LEDGER_FORMAT_VERSION)
            validateBlobHash(blob);
        else
            validateGitObject(blob);
        return {
            kind,
            blob,
            size: nonNegativeInteger(record, 'size'),
            mode,
            ...(version === GIT_CHECKPOINT_FORMAT_VERSION ? { provider: 'git' } : {}),
        };
    }
    if (kind === 'symlink') {
...
function parseGitCheckpoint(value, restorePointId) {
    const record = objectRecord(value, 'Git checkpoint metadata');
    const objectFormat = record.objectFormat;
    if (objectFormat !== 'sha1' && objectFormat !== 'sha256')
        corrupt('Git checkpoint objectFormat must be sha1 or sha256');
    const trust = record.trust;
    if (trust !== 'metadata-fenced' && trust !== 'byte-verified')
        corrupt('Git checkpoint trust is invalid');
    ...
    const object = (key) => {
        ...
        if (oid.length !== expectedLength)
            corrupt(`${key} length does not match Git object format`);
        return oid;
    };
...
function hexIdentifier(record, key) {
    const value = stringField(record, key);
    if (!/^[0-9a-f]{32}$/.test(value))
        corrupt(`${key} must be a 128-bit lowercase hexadecimal identifier`);
    return value;
}
function validateGitObject(value) {
    if (!GIT_OBJECT_PATTERN.test(value))
        corrupt('invalid Git object id');
    return value;
}
