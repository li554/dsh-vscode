...
export async function ensureGitWorktreeIdentity(cwd, signal) {
    const root = await discoverRepositoryRoot(cwd, signal);
...
export async function resolveGitWorktreeRoot(commonDir, worktreeId, fallback, signal) {
    ...
    try {
        ...
        for (const record of output.split('\0\0')) {
            ...
            if (first?.startsWith('worktree '))
                candidates.push(first.slice('worktree '.length));
        }
    }
    catch {
    ...
    for (const candidate of [...new Set(candidates)]) {
        try {
            ...
            if (identity.worktreeId === worktreeId && identity.commonDir === commonDir)
...
export async function runGit(cwd, args, signal, options = {}) {
...
export async function runGitOptional(cwd, args, signal, options = {}) {
    try {
...
export async function runGitInput(cwd, args, input, signal, options = {}) {
    return new Promise((resolvePromise, reject) => {
        ...
        const fail = (error) => {
            ...
            reject(new ChangeLedgerError('GIT_COMMAND_FAILED', `git ${args.join(' ')} failed in ${JSON.stringify(cwd)}: ${errorMessage(error)}`, { cause: error }));
        };
        child.on('error', fail);
        ...
        child.on('close', (code) => {
            ...
            if (code !== 0) {
                ...
                reject(new ChangeLedgerError('GIT_COMMAND_FAILED', `git ${args.join(' ')} failed in ${JSON.stringify(cwd)}: ${message || `exit ${String(code)}`}`));
                return;
            }
            if (stdoutBytes > GIT_MAX_BUFFER) {
                settled = true;
...
export async function runGitBuffer(cwd, args, signal, options = {}) {
    return new Promise((resolvePromise, reject) => {
        execFile('git', ['-c', 'core.quotepath=false', '-C', cwd, ...args], {
            encoding: 'buffer',
            maxBuffer: options.maxBuffer ?? GIT_MAX_BUFFER,
            ...
            if (error !== null) {
                ...
                reject(new ChangeLedgerError('GIT_COMMAND_FAILED', `git ${args.join(' ')} failed in ${JSON.stringify(cwd)}: ${stderr.toString('utf8').trim() || errorMessage(error)}`, { cause: error }));
                return;
            }
            resolvePromise(stdout);
        });
...
function parseWorktreeId(value) {
...
async function readGitWorktreeIdentity(cwd, signal) {
    const root = await discoverRepositoryRoot(cwd, signal);
    const gitDir = await realpath(resolve(root, stripLineEnding(await runGit(root, ['rev-parse', '--git-dir'], signal))));
    const commonDir = await realpath(resolve(root, stripLineEnding(await runGit(root, ['rev-parse', '--git-common-dir'], signal))));
...
async function ensureWorktreeId(identityDir, commonDir, gitDir) {
    ...
    if (gitDir !== commonDir && centralCopy.value !== undefined) {
        throw new ChangeLedgerError('STATE_CORRUPT', 'linked Git worktree identity is ambiguous because only a detached central copy remains');
    }
...
async function readIdentityCopy(path) {
    try {
        ...
        if (error instanceof ChangeLedgerError && error.code === 'STATE_CORRUPT')
            return { malformed: true };
        throw error;
    }
}
async function publishIdentityFile(path, value, repairMalformed) {
