...
// LOCAL PATCH for dsh-vscode: CREATE_NO_WINDOW (0x08000000). Without this
// every CreateProcessW console tool (git, pwsh, …) is given a visible console.
// Combined with STARTF_USESHOWWINDOW + SW_HIDE it keeps agent toolchains silent.
const CREATE_NO_WINDOW = 0x08000000;
/** CREATE_UNICODE_ENVIRONMENT | CREATE_SUSPENDED | CREATE_NO_WINDOW */
const CURRENT_TOKEN_JOB_CREATION_FLAGS = 0x400 | 0x4 | CREATE_NO_WINDOW;
//#endregion
...
function spawnPipedProcess(api, options) {
	...
	try {
		...
		if (createRestrictedProcess(api, options, buildCommandLine(options.command, options.args), CREATE_NO_WINDOW, startupInfo, processInfo) === 0) throwWin32(api, "CreateProcessAsUserW", api.getLastError(), `command: ${options.command}, cwd: ${options.cwd}`);
...
function spawnCurrentTokenJobProcess(api, options) {
	...
	return spawnJobProcess(api, options, () => targetCarrierHandles(api, options.stdio), "CreateProcessW", (startupInfo, processInfo) => api.createProcessW(options.applicationName, commandLine, null, null, 1, CURRENT_TOKEN_JOB_CREATION_FLAGS, environment, options.cwd, startupInfo, processInfo));
}
/**
