...
var JsonlSessionPersistence = class extends SessionPersistence {
	...
	*/
	...
	/**
	* Read one stored session's events by id, without needing a live handle.
	*
	* LOCAL PATCH for dsh-vscode. The bundled `@dsh-undo/*` plugins were built
	* against a newer persistence contract whose service exposes `inspect(id)`; this
	* backend (0.1.5-rc.2) does not, so the Archive Tasks surface failed with
	*   `sessionPersistence.inspect is not a function`
	* for every archived — i.e. non-live — session. Live sessions never reach it:
	* callers prefer `ctx.sessions.get(id)?.events` and only fall back here.
	*
	* Implemented on top of the API this backend does expose (`open` + `read`), so it
	* adds no new storage behavior. Callers only read `.events`.
	*
	* @param id - the stored session to read.
	* @param options - optional cancellation.
	* @returns the stored events, shaped like the contract the callers expect.
	*/
	async inspect(id, options) {
		options?.signal?.throwIfAborted();
		const handle = await this.open(id, "read", options);
		try {
			const { eventState, events } = await handle.read(0, Number.MAX_SAFE_INTEGER, options);
