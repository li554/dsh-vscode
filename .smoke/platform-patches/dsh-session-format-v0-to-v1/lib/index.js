...
//#endregion
//#region lib/types/plugin-compat.js
/**
 * LOCAL PATCH for dsh-vscode — tolerate v0 data written by third-party plugins.
 *
 * WHY. The released-v0 validators are exact, and the v0->v1 migration REFUSES a
 * Session carrying anything outside the released shape. That is the right default
 * for the released format, but this deployment bakes third-party plugins, and
 * plugins wrote into sessions in ways the released spec never sanctioned. The
 * refusal is total: one offending event makes the whole session unobservable, so
 * a user upgrading from a plugin-augmented build loses access to that history:
 *
 ...
 *   "dshFileReview"
 *
 * Measured on one real 30-session history, the strict validators refused 1251
 * events across 21 sessions, in four shapes:
 *   1. a content block carrying a plugin annotation (dsh-file-review's
 *      `dshFileReview`, on an empty text block appended to a tool result);
 *   2. a `kind:"plugin"` message source carrying an extra member
 *      (dsh-web-review's `snapshotId` on a snapshot source);
 *   3. a `kind:"plugin"` source pairing `summary` with a form other than
 *      `notice` (dsh-mnemon's `instructions` + summary);
 *   4. a `subagent/descriptor` at version 2 (the `sidechat` plugin) where the
 *      released spec froze on version 3.
 *
 * WHAT IS NOT RELAXED. Required members are still enforced, missing members and
 * unknown block/source KINDS still refuse, and every relaxation is logged once
 * per distinct shape so it is visible rather than silent. Only data whose author
 * is a plugin, and which the v1 target could not carry verbatim, is dropped or
 * coerced:
 *   - unknown members are DROPPED (the annotation belongs to a plugin with no v1
 *     rendering path, and the v1 target validator would reject it anyway);
 *   - a `summary` on a non-notice plugin form is DROPPED (it is a one-line UI
 *     label; keeping it would violate the v1 contract);
 *   - `subagent/descriptor` version 2 is COERCED to 3, because the rest of the
 *     payload validates unchanged under the continuable branch — the version
 *     field is the only difference.
 *
 * The source v0 artifact is never modified, so nothing here is destructive.
 */
/** Tolerances already reported, so each shape is logged once rather than per event. */
const reportedPluginTolerances = /* @__PURE__ */ new Set();
/**
 * Record one tolerated shape.
 * @param what - short description of the tolerance.
 * @param label - diagnostic subject where it was first seen.
 */
 ...
 * @param record - object to prune in place.
 * @param allowed - member names the released shape admits.
 * @param label - diagnostic subject.
 */
function dropUnreleasedMembers(record, allowed, label) {
	for (const key of Object.keys(record)) {
		if (allowed.includes(key)) continue;
		delete record[key];
		reportPluginTolerance(`dropped member ${JSON.stringify(key)}`, label);
	}
}
//#endregion
...
/**
 * Released members of each known v0 content block, keyed by block type. An
 * unknown block type is absent here and is left untouched for the switch's
 * default branch (which admits any members).
 */
const RELEASED_CONTENT_BLOCK_MEMBERS = /* @__PURE__ */ new Map([
	["text", ["type", "text"]],
	["reasoning", ["type", "text"]],
	["image", ["type", "attachment"]],
	["tool-call", ["type", "id", "name", "arguments"]],
...
function contentBlockValue(value, label, version) {
	...
	// Plugin tolerances: see lib/types/plugin-compat.js.
	const allowed = RELEASED_CONTENT_BLOCK_MEMBERS.get(block["type"]);
	if (allowed !== void 0) dropUnreleasedMembers(block, allowed, label);
	switch (block["type"]) {
		case "text":
		case "reasoning":
...
function pluginSourceValue(source, label) {
	...
	// Plugin tolerances: see lib/types/plugin-compat.js. Plugins wrote members the
	// released source shape never admitted (dsh-web-review's `snapshotId`), so they
	// are dropped rather than refusing the whole Session.
	dropUnreleasedMembers(source, ["kind", "plugin", ...optional], label);
	...
	else if (source["summary"] !== void 0) {
		// Plugin tolerance: the released rule is `summary` only under the notice
		// form, but dsh-mnemon paired it with `instructions`. `summary` is a
		// one-line UI label the v1 target would reject on a non-notice form, so it
		// is dropped instead of failing the session.
		delete source["summary"];
		reportPluginTolerance(`dropped "${"summary"}" on a plugin source whose form is ${JSON.stringify(form)}`, label);
	}
}
...
function subagentDescriptorValue(data, label) {
	// Plugin tolerance: see lib/types/plugin-compat.js. The released spec froze on
	// descriptor version 3, but the `sidechat` plugin wrote version 2 — and every
	// other member of those descriptors validates unchanged under the continuable
	// branch below, so the version field is the only difference and is bumped
	// rather than refusing the Session.
	if (data["version"] === 2) {
		data["version"] = 3;
		reportPluginTolerance("coerced subagent/descriptor version 2 to 3", label);
	}
