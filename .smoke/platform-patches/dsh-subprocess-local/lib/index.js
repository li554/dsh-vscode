...
function launchWindowsJob(spec, targetEnv, internals = {}) {
	...
	try {
		child = (internals.spawn ?? spawn)(command, [
			...
			// LOCAL PATCH for dsh-vscode. Without this every command run through the
			// managed Windows job runner is handed a fresh console, so a terminal
			// window appears when the command starts and vanishes when it ends. The
			// fallback path (spawnSubprocess, below) already passes
			// `windowsHide: platform === "win32"`; this, the path Windows normally
			// takes, simply omitted it. Ignored on non-Windows platforms.
			windowsHide: true
		});
	} finally {
