	...
	return class SettingsController extends _classSuper {
		...
		*/
		async openSettingsDocument(signal) {
			...
			if (isAborted(signal)) throw new RemoteError("gateway/cancelled", "settings document open was aborted", {});
			// LOCAL PATCH for dsh-vscode. The embedded bridge used to live in
			// dsh-host-apiproxy, which 0.1.5 deleted: when DSH_EMBEDDED=1 it wrote the
			// settings path on stdout for the containing VS Code extension to reveal in
			// its editor instead of handing the file to a native opener the user cannot
			// see. 0.1.5 dropped both the env check and the sentinel, so
			// "open configuration file" in the panel silently did nothing
			// (`openNativeTextFile` has no visible effect in this deployment). Restored
			// here, in the handler that took over the same job.
			if (process.env.DSH_EMBEDDED === "1") {
				try {
					process.stdout.write("[dsh-vscode:open-settings] " + path + "\n");
				} catch {}
				return { opened: true };
			}
			try {
