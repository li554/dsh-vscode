 ...
 */
import { readBoundedBody } from "./vision-client.js";
 ...
 */
export function buildModelsUrl(baseURL, apiStyle) {
 ...
 */
export async function probeModels(spec, apiKey, signal) {
 ...
 */
export function buildModelPingRequest(spec) {
 ...
 */
export async function testModelConnection(spec, apiKey, signal) {
 ...
 */
export async function handleModelTest(stored, overrides, resolveKey, signal) {
    ...
    if (typeof apiStyle === 'string' && apiStyle.trim().length > 0)
        candidate.apiStyle = apiStyle;
    ...
    try {
        ...
        return { ok: false, error: { code: 'rejected', message: error.message } };
    }
    let resolvedKey;
    try {
 ...
 */
export async function handleModelProbe(stored, overrides, resolveKey, signal) {
    const candidate = { ...stored };
    const baseURL = overrides['baseURL'];
    const apiStyle = overrides['apiStyle'];
    const draftKey = overrides['apiKey'];
