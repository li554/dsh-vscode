	...
	return {
		...
		settings: {
			...
			async openDocument(request, signal) {
				...
				// dsh-vscode embedded bridge: the host runs inside the VS Code
				// extension host, so the native `openTextFile` shell gesture would
				// open nothing the user can see. Instead emit the settings path on
				// stdout for the containing extension to reveal in its editor.
