...
*/
/**
* The standard reasoning-effort ladder offered to a profile-listed model that
* declares no `reasoningEfforts` of its own and whose catalog base is silent
* about reasoning. `off` maps to `null` ("send nothing") exactly as pi-ai's
* wire spelling expects; the ladder is the dsh-vscode default off/low/high/max.
* Every OTHER pi-ai level (minimal, medium, xhigh) is pinned to `null` too:
* pi-ai's getSupportedThinkingLevels treats an UNMAPPED standard level as
* supported (it falls back to provider defaults), so leaving minimal/medium
* unmapped would leak them back into the selector. Pinning them to `null`
* keeps the offered set exactly off/low/high/max.
* @returns a `thinkingLevelMap` to spread into a model.
*/
function defaultReasoningLadder() {
	return {
		off: null,
		minimal: null,
		low: "low",
...
* A declared dict translates to pi-ai's `thinkingLevelMap` with every level
* decided explicitly: declared levels carry their wire spelling, undeclared
* levels are pinned to `null` (unsupported). A user-declared entry that leaves
* `reasoningEfforts` out is made reasoning-configurable with the standard
* ladder rather than silently pinned to `reasoning:false` (which left the
* server half with nothing to drive); the installed catalog's own ladder is
* preserved when it declares one, and an explicitly non-reasoning catalog model
* is never given a ladder. Catalog-served entries (no profile models list)
* keep the catalog's own disposition untouched.
* @param provider - provider route key, for diagnostics.
* @param entry - the configured model entry.
* @param base - the installed catalog entry of the same id, when one exists.
* @param userDeclared - whether the model was listed in the profile's models
*   array (as opposed to served from the catalog).
* @returns the reasoning fields the materialized model carries.
*/
function resolveModelReasoning(provider, entry, base, userDeclared) {
	const efforts = entry.reasoningEfforts;
	if (efforts !== void 0) {
		if (efforts === false) return { reasoning: false };
		if (efforts === null || Object.keys(efforts).length === 0) invalid(provider, `model "${entry.id}" has an empty reasoningEfforts; declare the offered levels, set false for a non-reasoning model, or omit the field to use the default ladder`);
	...
	// dsh-vscode embedded default (v0.2.7): every model the profile lists in its
	// models array is expected to be reasoning-configurable. When it declares no
	// reasoningEfforts and the catalog is silent about reasoning — a genuine
	// custom model on a third-party / non-cataloged provider (no base), or a base
	// that carries no reasoning capability — surface the standard
	// off/low/high/max ladder so the selector shows usable effort levels
	// instead of nothing. A base that already declares its own reasoning ladder
	// is preserved; an explicitly non-reasoning catalog model is not touched.
	if (base?.reasoning === false) return { reasoning: false };
	if (userDeclared) {
		if (base?.thinkingLevelMap) return { reasoning: true };
		return { reasoning: true, thinkingLevelMap: defaultReasoningLadder() };
	}
	// Catalog-served entry (no profile models list): keep the catalog's own
	// capability rather than inventing a ladder.
...
function resolveRouteModels(request) {
	...
	const models = entries.map((entry) => {
		...
		return {
			...
			contextWindow,
			maxTokens,
			...resolveModelReasoning(provider, entry, base, configured.length > 0),
