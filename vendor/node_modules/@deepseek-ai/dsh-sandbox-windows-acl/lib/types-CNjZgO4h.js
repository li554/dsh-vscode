...
* asynchronous pipe draining and exit waiting. CREATE_NO_WINDOW is always set:
* when the host runs without a console (e.g. the VS Code extension host, where
* process.execPath is a GUI-subsystem Electron binary), a console-subsystem
* child would otherwise allocate its own VISIBLE console window. Stdio
* redirection is pipe-based and unaffected (verified: stdout/stderr capture and
* exit codes work under the restricted token on Windows 10/11).
...
function spawnSandboxed(api, token, options) {
	...
	if (api.createProcessAsUserW(token, null, commandLine, null, null, 1, 134217728, null, options.cwd, startupInfo, processInfo) === 0) {
		const win32Code = api.getLastError();
...
function spawnSandboxedInherited(api, token, options) {
	...
	const processInfo = allocProcessInfo();
	const commandLine = buildCommandLine(options.command, options.args);
	const created = api.createProcessAsUserW(token, null, commandLine, null, null, 1, 4 | 134217728, null, options.cwd, startupInfo, processInfo);
