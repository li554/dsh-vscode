window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** LOCAL PATCH: standard Low/High/Max map written when a row is marked a reasoning model. Composer then offers Default + these levels (no profile.reasoning defaultEffort). */
		const STANDARD_REASONING_EFFORTS = {
			low: "low",
			high: "high",
			max: "max"
		};
		/** Whether this row already claims image input. */
		function hasImageInput(model) {
			const input = model["input"];
			return Array.isArray(input) && input.includes("image");
		}
		/** Whether this row is configured as a reasoning model (Low/High/Max). */
		function isReasoningModel(model) {
			const efforts = model["reasoningEfforts"];
			return efforts !== false && efforts !== null && typeof efforts === "object" && !Array.isArray(efforts) && Object.keys(efforts).some((level) => level !== "off");
		}
		/**
		...
		*/
		function ModelListEditor(props) {
			const { models, onChange, probe, operations, t, disabled, onPersistModels } = props;
					...
					models.map((model, index) => (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [(0, react_jsx_runtime.jsxs)("div", {
							...
							children: [(0, react_jsx_runtime.jsxs)("label", {
								...
								className: ModelsSection_module_css_default["modelField"],
								style: { gridColumn: "span 1", justifyContent: "flex-end" },
								children: [(0, react_jsx_runtime.jsx)("span", {
									className: ModelsSection_module_css_default["modelFieldLabel"],
									...
									children: [(0, react_jsx_runtime.jsx)("input", {
										...
										disabled,
										checked: hasImageInput(model),
										onChange: (event) => {
											const next = models.map((row, at) => {
												if (at !== index) return { ...row };
												const copy = { ...row };
												if (event.target.checked) copy.input = ["text", "image"];
												else delete copy.input;
												return copy;
											});
											onChange(next);
											onPersistModels?.(next);
										}
									}), (0, react_jsx_runtime.jsx)("span", {
								...
								children: [(0, react_jsx_runtime.jsx)("span", {
									className: ModelsSection_module_css_default["modelFieldLabel"],
									children: t("modelReasoningModel")
								}), (0, react_jsx_runtime.jsxs)("span", {
									style: { display: "inline-flex", alignItems: "center", gap: "6px", minHeight: "36px" },
									children: [(0, react_jsx_runtime.jsx)("input", {
										type: "checkbox",
										"aria-label": `${t("modelReasoningModel")} ${index + 1}`,
										disabled,
										checked: isReasoningModel(model),
										onChange: (event) => {
											const next = models.map((row, at) => {
												if (at !== index) return { ...row };
												const copy = { ...row };
												if (event.target.checked) copy.reasoningEfforts = { ...STANDARD_REASONING_EFFORTS };
												else delete copy.reasoningEfforts;
												return copy;
											});
											onChange(next);
											onPersistModels?.(next);
										...
										style: { fontSize: "13px", color: "var(--dsw-alias-label-primary)" },
										children: t("modelReasoningModelHint")
									})]
								})]
							})]
						}) : null]
		...
		*/
		function ProviderEditor(props) {
			...
			*/
			const applyOnce = async (sourceDraft) => {
				const ns = namespace.ns;
				const baseDraft = sourceDraft ?? draft;
				const next = layout === "pi-ai" && stringAt(baseDraft, "apiKeyEnv") === void 0 && stringAt(fallback, "apiKeyEnv") === void 0 && keyValue.length > 0 ? schema.setPath(baseDraft, ["apiKeyEnv"], keyRef) : baseDraft;
			...
			*/
			const curatedFields = (family) => {
				...
				const catalogProps = {
					...
					// LOCAL PATCH: capability toggles write immediately so settings.yaml
					// updates without waiting for the card footer's Apply.
					onPersistModels: async (nextModels) => {
						const nextDraft = schema.setPath(draft, ["models"], nextModels);
						setDraft(nextDraft);
						setBusy(true);
						setFailure(void 0);
						try {
							const failure = await applyOnce(nextDraft);
							if (failure !== void 0) setFailure(failure);
						} finally {
							setBusy(false);
						}
					},
		...
		*/
		...
		const en = {
			...
			modelAdvanced: "Capacities & capabilities",
			addModel: "Add model",
			...
			modelImageInputHint: "Vision",
			modelReasoningModel: "Reasoning model",
			modelReasoningModelHint: "Low / High / Max in the composer",
		...
		const zh = {
			...
			modelImageInput: "支持图像",
			modelImageInputHint: "视觉",
			modelReasoningModel: "推理模型",
			modelReasoningModelHint: "对话框可选 Default / Low / High / Max",
			fetchModels: "获取可用模型",
			fetching: "正在询问提供方…",
